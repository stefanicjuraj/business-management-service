...
/**
 * The 'DepartmentBusiness' class extends the 'Business' class and serves as the
 * business logic layer responsible for managing department-related operations
 * within the company's application. It provides methods for retrieving,
 * updating, inserting, and deleting department information, acting as the
 * bridge between the service layer and data access layer.
 *
 * This class inherits the fundamental functionality from the 'Business' class
 * and extends it with operations specific to departments. It plays a crucial
 * role in enforcing business rules and ensuring the integrity of
...
public class DepartmentBusiness extends Business {
                ...
                try {
                        ...
                        if (departments.isEmpty() || !companyName.equals(BusinessConfig.COMPANY_NAME)) {
                                ...
                                return Response.status(Response.Status.OK)
                                                .entity("{\"message\": \"No departments found for the specified company: "
                                                                + companyName + ".\"}")
                                                .build();
                        }
                        ...
                        return Response.status(Response.Status.INTERNAL_SERVER_ERROR)
                                        .entity("{\"message\":\"Unable to connect to the database. Detailed message: "
                                                        + e.getMessage() + "\"}")
                                        .build();
                }
        }
        ...
        public Response get(String companyName, int deptId) {
                try {
                        ...
                        if (!companyName.equals(BusinessConfig.COMPANY_NAME)) {
                                ...
                                return Response.status(Response.Status.BAD_REQUEST) // Using BAD_REQUEST for invalid
                                                                                    // inputs.
                                                .entity("{\"error\":\"Invalid company name provided: " + companyName
                                                                + ". Expected: " + BusinessConfig.COMPANY_NAME + ".\"}")
                                                .build();
                        }
                        ...
                        if (department == null) {
                                ...
                                // ID.
                                return Response.status(Response.Status.NOT_FOUND) // Using NOT_FOUND for nonexistent
                                                                                  // resources.
                                                .entity("{\"error\":\"Department ID " + deptId
                                                                + " does not exist for company " + companyName + ".\"}")
                                                .build();
                        }
                        ...
                        return Response.status(Response.Status.INTERNAL_SERVER_ERROR)
                                        .entity("{\"error\":\"Database connection failed. Error details: "
                                                        + e.getMessage() + "\"}")
                                        .build();
                }
        }
                ...
                try {
                        ...
                        if (!companyName.equals(BusinessConfig.COMPANY_NAME)) {
                                ...
                                return Response.status(Response.Status.BAD_REQUEST)
                                                .entity("{\"error\":\"Invalid company name provided. Expected: '"
                                                                + BusinessConfig.COMPANY_NAME + "' but received: '"
                                                                + companyName + "'.\"}")
                                                .build();
                        }
        ...
        public Response update(String departmentJson) {
                ...
                try {
                        ...
                        if (existingDeptWithSameNo != null && (existingDeptWithSameNo.getDeptNo() == null
                                        ? updatedDepartment.getDeptNo() != null
                                ...
                                return Response.status(Response.Status.CONFLICT)
                                                .entity("{\"error\":\"Department number '"
                                                                + updatedDepartment.getDeptNo()
                        ...
                        return Response.status(Response.Status.BAD_REQUEST)
                                        .entity("{\"error\":\"Department update failed due to invalid JSON format: "
                                                        + e.getMessage() + "\"}")
                                        .build();
                }
        }
        ...
        public Response delete(String companyName, int deptId) {
                try {
                        ...
                        if (!companyName.equals(BusinessConfig.COMPANY_NAME)) {
                                ...
                                return Response.status(Response.Status.BAD_REQUEST)
                                                .entity("{\"error\":\"Invalid company name provided. Expected: '"
                                                                + BusinessConfig.COMPANY_NAME + "', but got: '"
                                                                + companyName + "'.\"}")
                                                .build();
                        }
                        ...
                        if (existingDept == null) {
                                ...
                                return Response.status(Response.Status.NOT_FOUND)
                                                .entity("{\"error\":\"Department with ID '" + deptId
                                                                + "' not found in company '" + companyName + "'.\"}")
                                                .build();
                        }
                        ...
                        return Response.status(Response.Status.OK)
                                        .entity("{\"success\":\"Department with ID '" + deptId
                                                        + "' successfully deleted from company '" + companyName
                                                        + "'.\"}")
                                        .build();
                } catch (Exception e) {
